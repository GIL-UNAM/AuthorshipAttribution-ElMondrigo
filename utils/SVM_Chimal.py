...
import re
import pandas as pd
from books_info import books, train_test_split_full
...
for type in marcadores:
   ...
   mask = full_metrics.index != 68787
   final_model.fit(full_metrics.loc[mask],[books[key][0] for key in full_metrics.loc[mask].index])
   print(best_model_type)
   print(final_model.classes_)
   print(final_model.predict_proba(full_metrics.loc[[68787]]))
   ...
   with open('SVM_results_Final.csv', 'a',newline='') as f:
